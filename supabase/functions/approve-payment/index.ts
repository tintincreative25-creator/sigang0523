    ...
    if (user && requestData.items) {
      ...
      if (orderError) {
        ...
        for (const item of requestData.items) {
          ...
          // products 테이블에서 재고 감소 (stock 필드가 있는 경우만)
          // stock 필드가 없을 수 있으므로 에러 처리를 추가
          try {
            const { data: productData, error: stockError } = await supabaseClient
              .from("products")
              .select("stock")
              .eq("id", productId)
              .maybeSingle();

            // stock 필드가 있고 값이 있으면 재고 감소
            if (!stockError && productData && productData.stock !== undefined && productData.stock !== null) {
            ...
            // stock 필드가 없으면 무시 (sold_out으로만 관리하는 경우)
          } catch (error) {
            // stock 필드 조회 실패 시 무시 (필드가 없을 수 있음)
            console.log("재고 필드 조회 실패 (stock 필드가 없을 수 있음):", error);
          }
        }
      }
    }
